...
zRPC_timer *zRPC_timer_schedule(zRPC_context *context, zRPC_timespec deadline, zRPC_runnable *runnable) {
...
void zRPC_timer_canael(zRPC_context *context, zRPC_timer *timer) {
    if (!context->timer_holder->initialized) {
        return;
    }
...
void zRPC_timer_init(zRPC_context *context) {
...
void zRPC_timer_shutdown(zRPC_context *context) {
...
void zRPC_timer_next_timeout(zRPC_context *context, zRPC_timespec *ts) {
    int res = 0;
...
static zRPC_timer *pop_one(zRPC_context *context, zRPC_timespec now) {
    zRPC_timer *timer;
    for (;;) {
...
static size_t pop_timers(zRPC_context *context, zRPC_timespec now) {
    size_t n = 0;
    zRPC_timer *timer;
...
int zRPC_timer_run_some_expired_timers_now(zRPC_context *context, zRPC_timespec now) {
    size_t n = 0;
...
int zRPC_timer_run_some_expired_timers(zRPC_context *context) {
